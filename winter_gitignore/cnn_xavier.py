...
weights = {
    ...
    'wc01': tf.Variable(initializer([5, 5, 1, 8])),
    
    'wc02': tf.Variable(initializer([5, 5, 8, 8])),
    'wc1': tf.Variable(initializer([5, 5, 8, 16])),
    # 5x5 conv, 32 inputs, 64 outputs
    'wc2': tf.Variable(initializer([5, 5, 16, 32])),
    # fully connected, 7*7*64 inputs, 1024 outputs
    'wd1': tf.Variable(initializer([7*7*32, 200])),
    # 1024 inputs, 10 outputs (class prediction)
    'out': tf.Variable(initializer([200, num_classes]))
}

biases = {
  
    'bc01': tf.Variable(initializer([8])),
    'bc02': tf.Variable(initializer([8])),
    'bc1': tf.Variable(initializer([16])),
    'bc2': tf.Variable(initializer([32])),
    'bd1': tf.Variable(initializer([200])),
...
loss_op = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=logits, labels=Y))+0.05*tf.nn.l2_loss(weights['wc1']) +0.05*tf.nn.l2_loss(weights['wc2'])+0.05*tf.nn.l2_loss(weights['wc01'])+0.05*tf.nn.l2_loss(weights['wc02'])+0.05*tf.nn.l2_loss(weights['wd1']) +0.05*tf.nn.l2_loss(weights['out'])
...
with tf.Session() as sess:
    summary_writer = tf.summary.FileWriter('/pylon5/at560kp/achatto2/Tensorflow/Clustering/ExtremeEvents/after_James/winter/correct_winter/log_xav', sess.graph)
