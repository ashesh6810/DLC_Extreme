...
#    conv4 = layers.Conv2D(filters=256, kernel_size=5, strides=1, padding='valid', activation='relu', name='conv4')(conv3)
    ...
    primarycaps = PrimaryCap(conv2, dim_capsule=8, n_channels=8, kernel_size=9, strides=2, padding='valid')
...
def margin_loss(y_true, y_pred, args):
    """
    ...
    m_upper=0.8
    m_lower=0.2
    L = y_true * K.square(K.maximum(0., m_upper - y_pred)) + \
        0.5 * (1 - y_true) * K.square(K.maximum(0., y_pred - m_lower))
...
#    (x_train, y_train), (x_test, y_test) = data
    ...
    model.fit([x_train, y_train], [y_train, x_train], steps_per_epoch=args.batch_size, epochs=args.epochs,
              validation_data=[[x_test, y_test], [y_test, x_test]], validation_steps=args.batch_size,callbacks=[log, tb, checkpoint, lr_decay])
...
def test(model, data, args):
    ...
    y_test=np.load('test_label.npy')
    y_pred, x_recon = model.predict(x_test,steps=32)
    print('-'*30 + 'Begin: test' + '-'*30)
    print('Test acc:', np.sum(np.argmax(y_pred, 1) == np.argmax(y_test, 1))/y_test.shape[0])
    np.savetxt("predicted_labels5x5.csv",y_pred,delimiter=",")
